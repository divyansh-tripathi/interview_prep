...
const Dashboard = () => {
  ...
  const inputCls = `rounded-2xl p-4 transition-all focus:outline-none focus:ring-2 ${isDark
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${isDark ? "bg-[#0a0e1a] text-white" : "bg-slate-50 text-slate-900"}`}>
      ...
      <div className="relative z-10 max-w-5xl mx-auto pt-10 sm:pt-12 px-4 pb-20">
        ...
        <div className={`p-6 sm:p-8 rounded-3xl border shadow-sm mb-10 transition-all ${isDark
        ...
        {sessions.length === 0 ? (
          <div className={`text-center py-24 rounded-3xl border border-dashed ${isDark ? "bg-white/5 border-white/10 text-white/60" : "bg-slate-100/60 border-slate-300 text-slate-500"
            }`}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5">
            {sessions.map((s) => (
              <div
                ...
                className={`group relative p-6 rounded-3xl border cursor-pointer transition-all duration-400 overflow-hidden hover:-translate-y-2 ${isDark
              ...
              >
                <div className={`w-10 h-10 rounded-2xl flex items-center justify-center text-white font-bold text-lg mb-4 shadow-lg bg-gradient-to-br ${isDark ? "from-blue-500 to-indigo-600" : "from-blue-500 to-indigo-500"
                  }`}>
                ...
                <div className={`mt-5 text-xs font-medium transition-colors ${isDark ? "text-blue-400/70 group-hover:text-blue-400" : "text-blue-500"
                  }`}>
                  Start Session →
                </div>
              </div>
